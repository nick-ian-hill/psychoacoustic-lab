...
var e = Object.create, t = Object.defineProperty, n = Object.getOwnPropertyDescriptor, r = Object.getOwnPropertyNames, i = Object.getPrototypeOf, a = Object.prototype.hasOwnProperty, o = (e, t) => () => (t || (e((t = { exports: {} }).exports, t), e = null), t.exports), s = (e, i, o, s) => {
	...
	constructor(e) {
		this.ctx = new (window.AudioContext || window.webkitAudioContext)(), this.seed = e, this.worker = new Worker(new URL(
			/* @vite-ignore */
			"" + new URL("assets/worker-BWMBPuZx.js", import.meta.url).href,
			"" + import.meta.url
	...
	async renderTrial(e, t, n, r, i, a = 1e4) {
		...
		return new Promise((c, l) => {
			let u = setTimeout(() => {
				this.pendingRequests.delete(o), l(/* @__PURE__ */ Error("Synthesis timeout: The audio worker took too long to respond."));
			}, a);
			this.pendingRequests.set(o, (e) => {
				clearTimeout(u), c(e);
	...
	async resume() {
		let e = this.ctx.resume(), t = new Promise((e) => setTimeout(e, 500));
		await Promise.race([e, t]);
	}
	async playBuffer(e, t) {
		await this.resume();
		let n = this.ctx.createBufferSource();
		n.buffer = e, n.connect(this.ctx.destination);
		let r = t ?? this.ctx.currentTime + this.getOutputLatency();
		return n.start(r), {
			source: n,
			startTime: r
		};
	...
	async close() {
		this.ctx.state !== "closed" && await this.ctx.close(), this.worker.terminate();
	}
	getTime() {
	...
	isFastStarting;
	constructor(e) {
		this.config = e || {}, this.currentValue = e ? e.initialValue : 0, this.currentStepIndex = 0, this.isFastStarting = e ? (e.initialN || 1) < e.rule.correctDown : !1;
	}
	getCurrentValue() {
	...
	processResponse(e, t) {
		let n = !1, r = this.currentValue;
		if (e) {
			this.consecutiveCorrect++;
			let e = this.config.rule ? this.isFastStarting ? this.config.initialN || 1 : this.config.rule.correctDown : Infinity;
			this.consecutiveCorrect >= e && this.config.stepSizes && (this.config.stepType === "geometric" ? this.currentValue /= this.getStepSize() : this.currentValue -= this.getStepSize(), this.consecutiveCorrect = 0, this.lastDirection === "up" && (n = !0, this.reversalCount++, this.handleReversal()), this.lastDirection = "down");
		} else this.config.stepSizes && (this.config.stepType === "geometric" ? this.currentValue *= this.getStepSize() : this.currentValue += this.getStepSize(), this.lastDirection === "down" && (n = !0, this.reversalCount++, this.handleReversal()), this.lastDirection = "up"), this.consecutiveCorrect = 0;
		this.config.minValue !== void 0 && (this.currentValue = Math.max(this.config.minValue, Math.min(this.config.maxValue, this.currentValue)));
		let i = {
		...
		return this.history.push(i), i;
	}
	getStepSize() {
		return this.config.stepSizes[Math.min(this.currentStepIndex, this.config.stepSizes.length - 1)];
	}
	handleReversal() {
		this.isFastStarting && this.reversalCount >= (this.config.switchReversalCount || 2) && (this.isFastStarting = !1);
		let e = this.config.stepSizeInterval || 1;
		this.reversalCount % e === 0 && this.config.stepSizes && this.currentStepIndex < this.config.stepSizes.length - 1 && this.currentStepIndex++;
	}
	isFinished(e) {
		return e ? !!(e.reversals && this.reversalCount >= e.reversals || e.trials && this.history.length >= e.trials || e.correctTrials && this.history.filter((e) => e.correct).length >= e.correctTrials) : !1;
	}
	calculateThreshold(e = 4) {
		let t = this.history.filter((e) => e.isReversal).slice(e);
		if (t.length === 0) return this.currentValue;
		let n = t.reduce((e, t) => e + t.value, 0) / t.length;
		if (this.config.stepType === "geometric") {
			let e = t.map((e) => e.value).filter((e) => e > 0);
			if (e.length === 0) return n;
			let r = e.reduce((e, t) => e + Math.log(t), 0);
			return Math.exp(r / e.length);
		}
		return n;
	}
	getHistory() {
...
function d(e, t = Math.random) {
	...
	return {
		targetIndex: i,
		intervalPerturbations: a
	};
}
//#endregion
//#region node_modules/seedrandom/lib/alea.js
var f = /* @__PURE__ */ o(((e, t) => {
	(function(e, t, n) {
		function r(e) {
			var t = this, n = o();
			t.next = function() {
	...
	(function(e, n, r) {
		...
		function p(e, t, o) {
			var d = [];
			t = t == 1 ? { entropy: !0 } : t || {};
			var f = _(g(t.entropy ? [e, b(n)] : e ?? y(), 3), d), p = new m(d), v = function() {
			...
			}, v.double = v, _(b(p.S), n), (t.pass || o || function(e, t, n, i) {
				return i && (i.S && h(i, p), e.state = function() {
					return h(p, {});
				}), n ? (r[s] = e, t) : e;
			})(v, f, "global" in t ? t.global : this == r, t.state);
		}
		function m(e) {
		...
		function y() {
			try {
				var t;
				return f && (t = f.randomBytes) ? t = t(i) : (t = new Uint8Array(i), (e.crypto || e.msCrypto).getRandomValues(t)), b(t);
			} catch {
	...
	constructor(e, t = {}) {
		this.container = e, this.options = t, this.engine = null, this.elements = {
			...
			e.key === "Escape" && this.currentConfig && !this.elements.experimentArea.classList.contains("hidden") && this.elements.resultsArea.classList.contains("hidden") && this.requestCancel();
		}, this.setupListeners();
	}
	findSafe(e) {
		let t = this.container.querySelector(`#${e}`) || this.container.querySelector(`.${e}`);
		if (!t) {
			let t = document.getElementById(e);
			if (t) return t;
			throw Error(`Required element not found: ${e}`);
		}
		return t;
	}
	findOptional(e) {
		try {
			return this.findSafe(e);
		} catch {
			return;
		}
	}
	setupListeners() {
		this.elements.playBtn.addEventListener("click", () => this.handlePlayClick()), this.elements.downloadBtn.addEventListener("click", () => this.handleDownload()), this.elements.quitBtn?.addEventListener("click", () => this.requestCancel()), document.addEventListener("keydown", this.keyDownHandler);
	}
	close() {
		this.engine &&= (this.engine.close(), null);
	}
	requestCancel() {
		if (!this.elements.experimentArea.classList.contains("hidden") && this.elements.resultsArea.classList.contains("hidden")) {
			if (!(this.sessionResults.length > 0 || this.currentBlockStartTime)) {
				this.cancel();
				return;
			}
			this.showModal("Cancel Experiment?", "Are you sure you want to stop the current experiment? All progress in this session will be lost.", "Stop Experiment", () => this.cancel());
		}
	}
	cancel() {
	...
	showModal(e, t, n, r, i = "Keep Going", a) {
		let o = this.isInputEnabled;
		this.isInputEnabled = !1;
		let s = document.createElement("div");
		s.className = "modal", s.innerHTML = `
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div style="display: flex; gap: 1rem; margin-top: 1.5rem;">
            <button class="btn" style="flex: 1; background: var(--psycho-keep-going-btn, var(--psycho-accent));" id="modal-cancel">${i}</button>
            <button class="btn" style="flex: 1; background: var(--psycho-stop-btn, var(--psycho-error));" id="modal-confirm">${n}</button>
          </div>
        </div>
      </div>
    `, this.container.appendChild(s);
		let c = () => {
			this.isInputEnabled = o, this.container.removeChild(s);
		};
		s.querySelector(".modal-close")?.addEventListener("click", () => {
			c(), a && a();
		}), s.querySelector("#modal-cancel")?.addEventListener("click", () => {
			c(), a && a();
		}), s.querySelector("#modal-confirm")?.addEventListener("click", () => {
			c(), r();
		});
	}
	async loadConfig(e) {
	...
	async initSession(t, n) {
		this.currentConfig = t, this.sessionResults = n ? n.results : [], this.engine && await this.engine.close(), this.elements.resultsArea.classList.add("hidden"), this.elements.playBtnContainer.classList.remove("hidden"), this.elements.instructionText.classList.remove("hidden"), this.elements.infoArea.classList.remove("hidden"), this.elements.mainArea.classList.remove("hidden"), this.activeSeed = n ? n.seed : t.meta.seed ?? Math.floor(Math.random() * 1e6), this.engine = new l(this.activeSeed), this.trialRng = (0, b.default)(this.activeSeed.toString()), this.blockQueue = e.flattenBlocks(t.blocks, this.trialRng);
		let r = n ? n.results.length : 0;
		r >= this.blockQueue.length ? this.showResults() : await this.startBlock(r);
	}
	...
	handleResponse(e) {
		...
		this.showFeedback(e, t), setTimeout(async () => {
			if (this.clearFeedback(), n) {
				let e = this.sessionResults.filter((e) => e.blockId === this.currentBlock?.id).length, t = {
				...
				this.sessionResults.push(t), this.container.dispatchEvent(new CustomEvent("block-complete", {
					detail: {
					...
					bubbles: !0,
					composed: !0
				})), this.currentBlockIndex < this.blockQueue.length - 1 ? (this.saveBackup(), await this.startBlock(this.currentBlockIndex + 1)) : this.showResults();
			} else {
	...
	clearFeedback() {
		this.responseButtons.forEach((e) => {
			e.classList.remove("correct", "incorrect", "active");
		});
	}
	updateStatus() {
		if (!this.staircase || !this.currentBlock || !this.currentConfig) return;
		let e = this.currentConfig.ui || {}, t = this.currentBlock.ui || {}, n = t.showTrialNumber ?? e.showTrialNumber ?? !0, r = t.showCurrentValue ?? e.showCurrentValue ?? !1, i = t.showReversals ?? e.showReversals ?? !1, a = this.staircase.getHistory().length + 1, o = this.staircase.getReversalCount(), s = this.staircase.getCurrentValue(), c = "";
		if (n && (c += `Trial: ${a}  `), i && (c += `Reversals: ${o}  `), r) {
			let e = this.currentBlock.adaptive?.unit || "";
			c += `Value: ${s.toFixed(2)}${e ? " " + e : ""}`;
		}
	...
	getBackup(e) {
		let t = localStorage.getItem(`psycho_lab_backup_${e}`);
		if (!t) return null;
		try {
			return JSON.parse(t);
		} catch {
	...
	saveBackup() {
		...
		localStorage.setItem(`psycho_lab_backup_${this.currentConfig.meta.name}`, JSON.stringify(e));
	}
	clearBackup(e) {
		localStorage.removeItem(`psycho_lab_backup_${e}`);
	}
}, S = ":host,:root{--lightningcss-light: ;--lightningcss-dark:initial;color-scheme:dark;--psycho-bg:#0f172a;--psycho-panel-bg:#1e293b;--psycho-text:#f8fafc;--psycho-text-muted:#94a3b8;--psycho-accent:#38bdf8;--psycho-accent-hover:#7dd3fc;--psycho-accent-secondary:#f472b6;--psycho-success:#10b981;--psycho-error:#ef4444;--psycho-border:#94a3b81a;--psycho-radius:4px}option{background-color:var(--psycho-panel-bg);color:var(--psycho-text)}*{box-sizing:border-box;-webkit-tap-highlight-color:transparent;margin:0;padding:0}body{background-color:var(--psycho-bg);color:var(--psycho-text);scrollbar-gutter:stable;-webkit-font-smoothing:antialiased;flex-direction:column;justify-content:center;align-items:center;min-height:100vh;font-family:Inter,-apple-system,sans-serif;line-height:1.5;display:flex}.container{background:var(--psycho-panel-bg);border-radius:var(--psycho-radius);border:1px solid var(--psycho-border);width:100%;max-width:600px;margin:0 auto;padding:2rem;position:relative;box-shadow:0 10px 15px -3px #0000004d}header{text-align:center;margin-bottom:1.5rem}h1{letter-spacing:-.025em;color:var(--psycho-text);margin-bottom:.5rem;font-size:1.75rem;font-weight:800}h1 span{color:var(--psycho-accent)}p.subtitle{color:var(--psycho-text-muted);font-size:.875rem}.control-group{flex-direction:column;gap:.75rem;margin-bottom:1.5rem;display:flex}label{color:var(--psycho-text-muted);font-size:.95rem;font-weight:500;display:block}.custom-select{-webkit-user-select:none;user-select:none;width:100%;position:relative}.select-trigger{background:var(--psycho-bg);border:1px solid var(--psycho-border);width:100%;color:var(--psycho-text);border-radius:var(--psycho-radius);cursor:pointer;justify-content:space-between;align-items:center;padding:.75rem 1rem;font-size:1rem;transition:all .2s;display:flex}.select-trigger:hover,.select-trigger:focus-visible{border-color:var(--psycho-accent);box-shadow:0 0 0 2px var(--psycho-badge-bg,#38bdf81a);outline:none}.select-trigger svg{width:1.25rem;height:1.25rem;color:var(--psycho-text-muted);transition:transform .2s}.custom-select.open .select-trigger svg{transform:rotate(180deg)}.select-options{background:var(--psycho-panel-bg);border:1px solid var(--psycho-border);border-radius:var(--psycho-radius);z-index:100;scrollbar-width:thin;scrollbar-color:var(--psycho-text-muted) transparent;max-height:35vh;animation:.2s ease-out slideDown;position:absolute;top:calc(100% + 4px);left:0;right:0;overflow-y:auto;box-shadow:0 10px 15px -3px #00000080}@keyframes slideDown{0%{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:translateY(0)}}.option{cursor:pointer;color:var(--psycho-text);-webkit-user-select:none;user-select:none;-webkit-touch-callout:none;touch-action:manipulation;padding:.75rem 1rem;font-size:.9375rem;transition:background .2s}.option:hover,.option.highlighted{background:var(--psycho-selection-bg,#3b82f61a);color:var(--psycho-accent)}.option.selected{background:var(--psycho-selection-selected,#3b82f633);color:var(--psycho-accent);font-weight:600}textarea{background:var(--psycho-bg);border:1px solid var(--psycho-border);width:100%;color:var(--psycho-text);border-radius:var(--psycho-radius);resize:vertical;min-height:120px;padding:.75rem;font-family:monospace;font-size:.875rem;line-height:1.6}textarea:focus{border-color:var(--psycho-accent);outline:none}.btn{background:var(--psycho-accent);color:var(--psycho-bg);border-radius:var(--psycho-radius);cursor:pointer;letter-spacing:.01em;text-align:center;border:1px solid #0000;justify-content:center;align-items:center;padding:1rem 2rem;font-size:1rem;font-weight:600;line-height:1.2;text-decoration:none;transition:all .2s;display:inline-flex}.btn:disabled{opacity:.5;cursor:not-allowed}.btn-full{width:100%}.btn-secondary{background:var(--psycho-secondary-btn,transparent);border:1px solid var(--psycho-text-muted);color:var(--psycho-text)}.btn-secondary:hover:not(:disabled){border-color:var(--psycho-text-muted);opacity:.9}.results-actions{justify-content:center;gap:1rem;width:100%;margin-top:1.5rem;display:flex}.results-actions .btn{flex:1;min-height:64px}.results-area{text-align:center;width:100%;margin-top:0;animation:.4s ease-out fadeIn}.results-area h2{color:var(--psycho-success);margin-bottom:1rem;font-size:1.5rem}.results-area p{color:var(--psycho-text);margin-bottom:1.5rem;font-size:1.125rem;font-weight:500}.experiment-area{flex-direction:column;align-items:center;gap:1.5rem;margin-top:0;padding-top:.25rem;display:flex}.selection-description{border-left:3px solid var(--psycho-accent);color:var(--psycho-text-muted);scrollbar-width:thin;background:#94a3b80d;border-radius:0;max-height:200px;margin-top:0;padding:1rem;font-size:.9375rem;line-height:1.6;overflow-y:auto}.experiment-info{flex-direction:column;justify-content:center;align-items:center;gap:1rem;width:100%;display:flex}.status-container{align-items:center;gap:.5rem;display:flex;position:relative}.instruction-line{justify-content:center;align-items:center;gap:.5rem;width:100%;display:flex}#instruction-text{text-align:center;color:var(--psycho-text-muted);white-space:pre-wrap;margin:0;font-size:.875rem;line-height:1.4}.experiment-main{place-items:center;width:100%;display:grid;position:relative}.play-btn-container,.response-buttons{grid-area:1/1;justify-content:center;align-items:center;gap:1rem;width:100%;transition:opacity .2s;display:flex}.play-btn-container:not(.hidden)~.response-buttons{opacity:0;pointer-events:none;visibility:hidden}.play-btn-container .btn{pointer-events:auto}.status-badge{background:var(--psycho-badge-bg,#38bdf81a);color:var(--psycho-accent);text-align:center;border-radius:999px;width:fit-content;max-width:100%;margin:0 auto;padding:.5rem 1.25rem;font-size:.875rem;font-weight:600;line-height:1.4;display:block}.response-buttons{gap:1rem;width:100%;display:flex}.response-btn{background:var(--psycho-interval-bg,var(--psycho-bg));border:2px solid var(--psycho-border);color:var(--psycho-interval-text,var(--psycho-text));border-radius:var(--psycho-radius);cursor:pointer;-webkit-tap-highlight-color:transparent;touch-action:manipulation;-webkit-user-select:none;user-select:none;flex:1;padding:1.5rem;font-size:1.5rem;font-weight:800;outline:none!important}.response-btn:focus,.response-btn:active,.response-btn:focus-visible{border-color:var(--psycho-border);box-shadow:none!important;outline:none!important;transform:none!important}.response-btn.non-selectable{cursor:default;pointer-events:none;opacity:.15;flex:.4;padding:.5rem}.response-btn.active{border-color:var(--psycho-accent);background:#38bdf81a;box-shadow:0 0 20px #38bdf866;opacity:1!important}.response-btn.correct{color:#22c55e;background:#22c55e1a;border-color:#22c55e;opacity:1!important}.response-btn.incorrect{color:#ef4444;background:#ef44441a;border-color:#ef4444;opacity:1!important}.response-btn.non-selectable.active{opacity:1}@media (hover:hover) and (pointer:fine){.btn:hover:not(:disabled){background:var(--psycho-accent-hover)}.response-btn:hover:not(:disabled){border-color:var(--psycho-accent);box-shadow:0 0 15px #38bdf833}}.response-btn:disabled{opacity:.2;cursor:not-allowed;border-color:var(--psycho-border);transform:none!important}.hidden{display:none!important}@keyframes pulse{0%{transform:scale(1)}50%{transform:scale(1.05)}to{transform:scale(1)}}.playing{opacity:.5}.response-btn.active{border-color:var(--psycho-accent);color:#fff;opacity:1;background:#38bdf81f;box-shadow:0 0 20px #38bdf873,inset 0 0 12px #38bdf81a}.response-btn.correct,.response-btn.correct:disabled{border-color:var(--psycho-success);color:var(--psycho-success);background:var(--psycho-success-bg,#10b98126);opacity:1}.response-btn.incorrect,.response-btn.incorrect:disabled{border-color:var(--psycho-error);color:var(--psycho-error);background:var(--psycho-error-bg,#ef444426);opacity:1}.modal{z-index:1000;justify-content:center;align-items:center;width:100%;height:100%;animation:.2s ease-out fadeIn;display:flex;position:fixed;top:0;left:0}.modal-overlay{background:var(--psycho-modal-overlay,#000000b3);-webkit-backdrop-filter:blur(4px);backdrop-filter:blur(4px);width:100%;height:100%;position:absolute;top:0;left:0}.modal-content{background:var(--psycho-panel-bg);border-radius:var(--psycho-radius);border:1px solid var(--psycho-border);flex-direction:column;width:90%;max-width:500px;max-height:80vh;display:flex;position:relative;overflow:hidden;box-shadow:0 25px 50px -12px #00000080}.modal-header{border-bottom:1px solid var(--psycho-border);background:#ffffff05;justify-content:space-between;align-items:center;padding:1.25rem 1.5rem;display:flex}.modal-header h2{color:var(--psycho-accent);margin:0;font-size:1.1rem}.modal-close{color:var(--psycho-text-muted);cursor:pointer;background:0 0;border:none;padding:0 .5rem;font-size:1.75rem;line-height:1;transition:color .2s}.modal-close:hover{color:var(--psycho-text)}.modal-body{color:var(--psycho-text);padding:1.5rem;font-size:.9375rem;line-height:1.6;overflow-y:auto}@keyframes fadeIn{0%{opacity:0}to{opacity:1}}@media (width<=640px){body{justify-content:center;padding:1rem}.container{flex-direction:column;width:100%;max-width:none;min-height:480px;margin:0;padding:1.5rem;display:flex}header{margin-bottom:1.5rem}h1{font-size:1.5rem}.experiment-area{gap:1rem;margin-top:0;padding-top:.25rem}.experiment-info{min-height:80px}.modal-content{width:95%;max-height:90vh}.experiment-main{min-height:140px}.response-btn{padding:1.25rem .75rem;font-size:1.1rem}}@media (height<=500px){body{justify-content:center;padding:.5rem}.container{min-height:auto;padding:1rem}header{margin-bottom:1rem}.experiment-area{gap:.5rem;margin-top:.5rem;padding-top:.5rem}}.quit-btn{top:var(--psycho-quit-top,.15rem);right:var(--psycho-quit-right,.15rem);color:var(--psycho-text-muted);width:var(--psycho-quit-width,40px);height:var(--psycho-quit-height,40px);cursor:pointer;z-index:10;background:0 0;border:none;border-radius:50%;justify-content:center;align-items:center;padding:.5rem;font-size:1.25rem;line-height:1;transition:all .2s;display:flex;position:absolute}.quit-btn:hover{color:var(--psycho-error);background:#ef44441a}.quit-btn:focus-visible{outline:2px solid var(--psycho-accent);outline-offset:2px}", C = {
	pitchDiscrimination: {
		meta: {
		...
		blocks: [{
			...
			id: "test",
			feedback: !0,
			meta: { summary: "Great! You're ready for the real test. Identify the HIGHER pitch." },
			ui: { showTrialNumber: !1 },
			paradigm: {
				type: "m-AFC",
				intervals: [{ condition: "reference" }, { condition: "target" }],
				randomizeOrder: !0,
	...
	connectedCallback() {
		...
		t && this.loadRemoteConfig(t), this.hasAttribute("show-quit") && this.shadowRoot.getElementById("quit-btn")?.classList.remove("hidden"), this.shadowRoot.getElementById("download-results-btn")?.addEventListener("click", () => this.handleDownload()), this.shadowRoot.getElementById("finish-btn")?.addEventListener("click", () => this.cancel());
	}
	render() {
	...
	async loadRemoteConfig(e) {
		console.log(`Loading config: ${e}`);
	}
	setConfig(e) {
...
var E = class extends HTMLElement {
	...
	setupListeners() {
		let e = this.shadowRoot.getElementById("dropdown-trigger"), t = this.shadowRoot.getElementById("dropdown-options"), n = this.shadowRoot.getElementById("custom-dropdown"), r = this.shadowRoot.getElementById("selected-text"), i = this.shadowRoot.getElementById("load-btn"), a = this.shadowRoot.getElementById("selection-screen"), o = this.shadowRoot.getElementById("runner-screen"), s = this.shadowRoot.getElementById("the-runner"), c = this.shadowRoot.getElementById("selection-description"), l = this.shadowRoot.getElementById("custom-json-group"), u = this.shadowRoot.getElementById("custom-file"), d = -1, f = Array.from(t.querySelectorAll(".option")), p = (e) => {
			f.forEach((t, n) => {
				t.classList.toggle("highlighted", n === e);
			}), e >= 0 && f[e].scrollIntoView({ block: "nearest" });
		};
		e.addEventListener("click", (e) => {
			e.stopPropagation(), t.classList.contains("hidden") ? (t.classList.remove("hidden"), n.classList.add("open"), d = f.findIndex((e) => e.classList.contains("selected")), p(d)) : (t.classList.add("hidden"), n.classList.remove("open"));
		}), document.addEventListener("keydown", (r) => {
			let i = this.shadowRoot.activeElement === e, a = t.classList.contains("hidden");
			if (a && i && (r.key === "Enter" || r.key === " ")) {
				r.preventDefault(), e.click();
				return;
			}
			a || (r.key === "ArrowDown" ? (r.preventDefault(), d = Math.min(d + 1, f.length - 1), p(d)) : r.key === "ArrowUp" ? (r.preventDefault(), d = Math.max(d - 1, 0), p(d)) : r.key === "Enter" || r.key === " " ? (r.preventDefault(), d >= 0 && f[d].click()) : r.key === "Escape" && (t.classList.add("hidden"), n.classList.remove("open")));
			...
			let i = (i) => {
				...
				i.stopPropagation();
				let a = e.getAttribute("data-value") || "", o = e.textContent || "";
				this.selectedValue = a, r.textContent = o, f.forEach((e) => e.classList.remove("selected")), e.classList.add("selected"), t.classList.add("hidden"), n.classList.remove("open"), l.classList.toggle("hidden", a !== "custom"), c.textContent = C[a]?.meta?.description || (a === "custom" ? "Upload a valid ExperimentConfig JSON file to begin." : "No description available.");
			};
			e.addEventListener("click", i), e.addEventListener("pointerup", (e) => {
				e.pointerType !== "mouse" && i(e);
			});
		...
		let m = C[this.selectedValue];
		m?.meta?.description && (c.textContent = m.meta.description);
	}
};
